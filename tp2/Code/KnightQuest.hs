...
-- 0 0 -> 1 0
exec :: Int
exec = execAux initialMove (0,0) 0

execAux :: [(Int,Int)] -> (Int,Int) -> Int -> Int
execAux l p n = if targetAchieved p l then n
                else execAux (concatMap (\x -> possibleMoves x) l) p (n + 1)
